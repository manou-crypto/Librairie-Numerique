...
export default function DashboardBottomRow() {
  ...
  useEffect(() => {
    financesService.getDashboardFeed()
  ...
  if (loading) {
    return <div className="h-48 flex items-center justify-center text-muted-foreground"><Loader2 className="animate-spin" /></div>;
  }

  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-4">
      <div className="lg:col-span-2"><TopProductsTable products={feed?.topProducts || []} /></div>
      <div><RecentSalesFeed sales={feed?.recentSales || []} /></div>
    </div>
  );
}
