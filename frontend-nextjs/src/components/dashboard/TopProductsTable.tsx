        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              ...
              <th className="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wide">CA (FCFA)</th>
          ...
          <tbody>
            {topProducts.map((p) => (
              <tr key={p.id} className="table-row-hover border-b border-border last:border-0">
                ...
                <td className="px-4 py-3 text-right"><span className="font-bold tabular-nums text-foreground text-xs">{p.ca.toLocaleString('fr-FR', { minimumFractionDigits: 2 })} FCFA</span></td>
                <td className="px-4 py-3 text-right"><span className={`text-xs font-semibold tabular-nums ${p.marge > 40 ? 'text-green-600' : p.marge > 30 ? 'text-blue-600' : 'text-amber-600'}`}>{p.marge}%</span></td>
