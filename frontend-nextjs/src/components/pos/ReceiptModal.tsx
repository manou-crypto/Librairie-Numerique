  ...
  return (
    <Modal open={open} onClose={onClose} title="Impression du Reçu" size="sm">
      <div className="space-y-6">
        ...
        <div
          className="relative mx-auto w-full max-w-[320px] bg-[#f9fafb] text-slate-900 px-6 py-8 rounded-sm shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-200 overflow-hidden"
        ...
        >
          ...
          <div className="font-mono text-xs space-y-4">
            ...
            <div className="border-t-2 border-dashed border-slate-300 pt-3 pb-2 space-y-1.5 text-slate-700">
              ...
              <div className="flex justify-between">
                <span>TICKET N°</span>
                <span>{referenceTicket || saleId}</span>
              </div>
              <div className="flex justify-between">
                <span>CAISSE</span>
                <span>{caisse}</span>
                ...
                <div key={idx} className="flex justify-between items-start leading-tight pt-1">
                  <div className="flex-1 pr-4">
                    <span className="block font-medium truncate">
                      {item.nomKit ? `[Kit: ${item.nomKit}] ` : ''}
                      {item.name}
                    </span>
                    <span className="text-[10px] text-slate-500">
                      {item.qty} x {item.price.toLocaleString('fr-FR')}
                    </span>
                  </div>
