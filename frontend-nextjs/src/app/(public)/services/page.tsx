    ...
    <div className="min-h-screen bg-[#FAFAF8] dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 flex items-center justify-between gap-6">
            ...
            <nav className="hidden lg:flex items-center gap-6 text-[13px] font-medium text-slate-700 dark:text-slate-300">
              ...
              <Link
                href="/services"
                className="text-slate-950 dark:text-white font-semibold border-b-2 border-slate-900 dark:border-white pb-0.5"
              >
                A Propos
              </Link>
            </nav>
      ...
      <section className="relative w-full overflow-hidden bg-slate-950 text-white">
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1507842229456-1cb7d55f0535?auto=format&fit=crop&w=2000&q=80')",
          }}
        />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28 lg:py-36">
          ...
          <div className="max-w-3xl space-y-6">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight uppercase leading-[1.08]">
              <span className="text-white">{nomLibrairie}</span>
              <br />
              <span className="text-slate-400 text-3xl sm:text-4xl lg:text-5xl font-semibold normal-case">
                Bien plus qu une librairie.
              </span>
            </h1>
            <p className="text-base sm:text-lg text-slate-300/90 font-light max-w-2xl leading-relaxed">
              Depuis plus de 15 ans, nous accompagnons etudiants, familles et professionnels dans
              leur passion de la lecture. Trois points de vente, un catalogue en ligne, et une
              equipe passionnee a votre service.
            </p>
            <div className="flex flex-wrap gap-3 pt-2">
              ...
              <a
                href="#contact"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-md bg-white/10 hover:bg-white/20 text-white text-xs sm:text-sm font-bold tracking-wider uppercase border border-white/20 transition-all"
              >
                <Mail size={15} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
              <p>
                Fondee il y a plus de 15 ans,{' '}
                <strong className="text-slate-900 dark:text-white">{nomLibrairie}</strong> est
                devenue une reference incontournable pour les amateurs de livres, les etudiants et
                les professionnels de la region.
              </p>
              <p>
                Notre mission est simple : rendre le savoir accessible a tous, a travers une
                selection rigoureuse d ouvrages allant des romans contemporains aux manuels
                universitaires, en passant par les essais, les bandes dessinees et les livres de
                developpement personnel.
              </p>
              <p>
                Avec notre catalogue numerique, vous pouvez desormais consulter notre stock en temps
                reel et trouver l ouvrage qu il vous faut avant meme de vous deplacer.
              </p>
            </div>
            <Link
              href="/catalogue"
              className="inline-flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-white group"
            >
      ...
      <section
        id="points-de-vente"
        className="bg-[#F4F4F0] dark:bg-slate-900/50 border-y border-slate-200/80 dark:border-slate-800 py-16 sm:py-20 scroll-mt-20"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              Retrouvez-nous dans nos trois espaces dedies a la lecture et au savoir.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {POINTS_DE_VENTE.map((pdv) => (
              <div
                key={pdv.id}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-6 shadow-xs hover:shadow-md transition-all space-y-4"
              >
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="mt-3 text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                Que ce soit pour une commande speciale, une demande d information sur nos
                disponibilites ou simplement un conseil de lecture, notre equipe est disponible pour
                vous.
              </p>
            </div>
            <div className="space-y-4">
              {[
                ...
                return (
                  <div
                    key={item.label}
                    className="flex items-center gap-3 p-4 rounded-xl bg-[#F4F4F0] dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800"
                  >
                    <div className="w-9 h-9 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center shrink-0 text-slate-700 dark:text-slate-300">
                      <Icon size={16} />
                    </div>
                    <div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 bg-[#FAFAF8] dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-slate-400 dark:focus:border-slate-500 transition-colors"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-[#FAFAF8] dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-slate-400 dark:focus:border-slate-500 transition-colors"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 bg-[#FAFAF8] dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-slate-400 dark:focus:border-slate-500 transition-colors resize-none"
                  />
                </div>
                <button
                  type="submit"
                  disabled={sending}
                  className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:hover:bg-white text-white dark:text-slate-900 text-sm font-semibold transition-all shadow-xs disabled:opacity-60"
                >
                  {sending ? (
                    <>
      ...
      <section className="bg-slate-950 text-white py-14 sm:py-18">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-[11px] font-semibold tracking-wider text-slate-300 uppercase">
            <Package size={12} />
            <span>Stock en temps reel</span>
          </div>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight">
            Plus de 2 000 ouvrages a decouvrir
          </h2>
          <p className="text-slate-400 text-sm sm:text-base max-w-xl mx-auto">
            Consultez notre catalogue numerique pour trouver le livre qu il vous faut -
            disponibilite en temps reel, recherche par titre, auteur ou categorie.
          </p>
          <Link
            href="/catalogue"
            className="inline-flex items-center gap-2 px-8 py-3.5 rounded-md bg-[#E14B1C] hover:bg-[#C83E13] text-white font-black text-sm tracking-wider uppercase shadow-lg transition-transform hover:scale-[1.02] active:scale-[0.98]"
          >
            <span>Explorer le catalogue</span>
