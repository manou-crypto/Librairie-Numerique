        ...
        <div className="flex items-start gap-4 p-4 bg-red-50 border border-red-200 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs text-red-700">
              Le produit et toutes ses données associées seront définitivement supprimés du système.
              Les ventes passées ne seront pas affectées.
            </p>
          </div>
        </div>
        {product && (
          <div className="bg-muted/50 rounded-xl p-4 border border-border">
            <p className="text-xs text-muted-foreground mb-1">Produit à supprimer :</p>
            <p className="text-sm font-bold text-foreground">{product.name}</p>
            <p className="text-xs text-muted-foreground font-mono mt-0.5">{product.reference}</p>
            <p className="text-xs text-muted-foreground mt-1">
              Stock actuel : {product.stock} unités · {product.categoryName}
            </p>
          </div>
        )}
