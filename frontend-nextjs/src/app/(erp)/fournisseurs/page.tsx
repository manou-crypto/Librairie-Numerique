...
export default function FournisseursPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setFournisseurs(await fournisseursService.getAll());
    } catch {
  ...
  const openEdit = (f: Fournisseur) => {
    ...
    setForm({
      ...
      ville: f.ville || '',
      pays: f.pays || '',
      numeroContribuable: f.numeroContribuable || '',
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info">
            ...
            <p className="text-xl font-bold text-foreground tabular-nums">{fournisseurs.length}</p>
            <p className="text-xs text-muted-foreground mt-1">enregistrés</p>
          </div>
          <div className="kpi-card-positive">
            <p className="text-xs text-muted-foreground mb-1">Avec contact</p>
            <p className="text-xl font-bold text-foreground tabular-nums">
              {fournisseurs.filter((f) => f.contactNom).length}
            </p>
            <p className="text-xs text-muted-foreground mt-1">fiches complètes</p>
          </div>
          <div className="kpi-card-neutral">
            <p className="text-xs text-muted-foreground mb-1">Avec email</p>
            <p className="text-xl font-bold text-foreground tabular-nums">
              {fournisseurs.filter((f) => f.email).length}
            </p>
          </div>
          <div className="kpi-card-warning">
            <p className="text-xs text-muted-foreground mb-1">Sans téléphone</p>
          ...
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-lg fade-in">
            ...
            <div className="px-6 py-5 grid grid-cols-2 gap-4">
              {[
                ...
                [
                  'Ville / Pays',
                  selected.ville ? `${selected.ville}, ${selected.pays || ''}` : selected.pays,
                ],
              ].map(([lbl, val]) => (
                <div key={lbl}>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
              <button onClick={() => setSelected(null)} className="btn-secondary text-sm py-2">
                Fermer
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-sm fade-in p-6 space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Cette action est irréversible. Les bons d'achat associés ne seront pas supprimés.
            </p>
