...
export default function NotificationsPage() {
  ...
  const tabs: { key: FilterTab; label: string }[] = [
    ...
    { key: 'alertes', label: 'Alertes' }, { key: 'ventes', label: 'Ventes' }, { key: 'stock', label: 'Stock' },
  ];
    ...
    <AppLayout currentPath="/notifications">
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info"><p className="text-xs text-muted-foreground mb-1">Total notifications</p><p className="text-2xl font-bold text-foreground tabular-nums">{notifs.length}</p></div>
          <div className="kpi-card-warning"><p className="text-xs text-muted-foreground mb-1">Non lues</p><p className="text-2xl font-bold text-warning tabular-nums">{nonLues}</p></div>
          <div className="kpi-card-neutral"><p className="text-xs text-muted-foreground mb-1">Alertes stock</p><p className="text-2xl font-bold text-foreground tabular-nums">{notifs.filter(n => n.type === 'stock').length}</p></div>
          <div className="kpi-card-positive"><p className="text-xs text-muted-foreground mb-1">Lues</p><p className="text-2xl font-bold text-positive tabular-nums">{notifs.filter(n => n.lue).length}</p></div>
        </div>
        <div className="card-base overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-1 overflow-x-auto">
              {tabs.map(tab => (<button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${activeTab === tab.key ? 'bg-primary text-white' : 'text-muted-foreground hover:text-foreground hover:bg-muted'}`}>{tab.label}</button>))}
            </div>
            <div className="flex items-center gap-2 shrink-0">
              {nonLues > 0 && <button onClick={markAllRead} className="btn-secondary flex items-center gap-1.5 text-xs py-1.5"><CheckCheck size={13} /> Tout marquer lu</button>}
              {notifs.length > 0 && <button onClick={clearAll} className="btn-secondary flex items-center gap-1.5 text-xs py-1.5 text-negative border-negative/30 hover:bg-negative/5"><Trash2 size={13} /> Tout effacer</button>}
            </div>
          </div>
