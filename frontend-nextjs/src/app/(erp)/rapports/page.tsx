...
import { Download, TrendingUp, ShoppingBag, Package, DollarSign, Calendar, Loader2 } from 'lucide-react';
...
export default function RapportsPage() {
  ...
  useEffect(() => {
    ...
    rapportsService.getStats(period)
      .then(data => {
        if (isMounted) {
          setStats(data);
          setLoading(false);
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info">
            ...
            <p className="text-2xl font-bold text-foreground tabular-nums">{stats.chiffreAffaires.toLocaleString('fr-FR')} FCFA</p>
          ...
          <div className="kpi-card-positive">
            ...
            <p className="text-2xl font-bold text-positive tabular-nums">{stats.beneficeNet.toLocaleString('fr-FR')} FCFA</p>
            <p className="text-xs text-positive mt-1">Données temps réel</p>
          </div>
          <div className="kpi-card-neutral">
            ...
            <p className="text-xs text-muted-foreground mt-1">transactions ({periodLabels[period].toLowerCase()})</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 card-base p-5">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={stats.caData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="colorCA2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563eb" stopOpacity={0.15} /><stop offset="95%" stopColor="#2563eb" stopOpacity={0} /></linearGradient>
                  <linearGradient id="colorBen2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#22c55e" stopOpacity={0.15} /><stop offset="95%" stopColor="#22c55e" stopOpacity={0} /></linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
                <XAxis dataKey="mois" tick={{ fontSize: 11, fill: '#64748b' }} axisLine={false} tickLine={false} dy={10} />
                <YAxis tick={{ fontSize: 11, fill: '#64748b' }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} dx={-10} />
                <Tooltip 
                  formatter={(v: number) => `${v.toLocaleString('fr-FR')} FCFA`}
                  contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} 
                />
                <Area type="monotone" dataKey="ca" stroke="#2563eb" strokeWidth={3} fill="url(#colorCA2)" name="CA" />
                <Area type="monotone" dataKey="benefice" stroke="#22c55e" strokeWidth={3} fill="url(#colorBen2)" name="Bénéfice" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="card-base p-5">
            ...
            {stats.categoryData.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={stats.categoryData} cx="50%" cy="50%" innerRadius={60} outerRadius={85} paddingAngle={2} dataKey="value" stroke="none">
              ...
              <div className="flex h-[220px] items-center justify-center text-sm text-muted-foreground">Aucune donnée sur la période</div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card-base p-5">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={stats.ventesJour} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
                <XAxis dataKey="jour" tick={{ fontSize: 11, fill: '#64748b' }} axisLine={false} tickLine={false} dy={10} />
                <YAxis tick={{ fontSize: 11, fill: '#64748b' }} axisLine={false} tickLine={false} dx={-10} allowDecimals={false} />
                <Tooltip 
                  cursor={{ fill: 'transparent' }}
                  contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} 
                />
                <Bar dataKey="ventes" fill="#2563eb" radius={[4, 4, 0, 0]} name="Ventes (Qté)" maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="card-base overflow-hidden">
            ...
            {stats.topProducts.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      ...
                      <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">CA (FCFA)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {stats.topProducts.map((p, idx) => (
                      <tr key={idx} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                        ...
                        <td className="px-5 py-3">
                          <p className="font-medium text-foreground text-sm line-clamp-1">{p.nom}</p>
                        ...
                        <td className="px-5 py-3 text-right font-semibold text-primary tabular-nums">{p.ca.toLocaleString('fr-FR')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex h-[200px] items-center justify-center text-sm text-muted-foreground">Aucune vente sur la période</div>
            )}
          </div>
        </div>
