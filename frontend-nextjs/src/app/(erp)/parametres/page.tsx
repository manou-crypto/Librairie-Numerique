...
import { Save, Store, Globe, Bell, Shield, Database, Upload, CheckCircle, Download, Camera } from 'lucide-react';
...
export default function ParametresPage() {
  ...
  const [general, setGeneral] = useState({ nom_librairie: '', logo_url: '', devise: 'DZD', tva: 0 });
        ...
        {activeTab === 'general' && (
          <div className="card-base p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1.5">Nom de la librairie</label>
                <input type="text" value={general.nom_librairie} onChange={(e) => setGeneral({ ...general, nom_librairie: e.target.value })} className="input-field text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1.5">URL du Logo (optionnel)</label>
                <div className="flex gap-2">
                  <input type="url" placeholder="https://..." value={general.logo_url} onChange={(e) => setGeneral({ ...general, logo_url: e.target.value })} className="input-field text-sm" />
                </div>
              </div>
            </div>
            <div className="border-t border-border pt-5">
              <h3 className="text-sm font-bold text-foreground mb-4 flex items-center gap-2"><Globe size={16} className="text-primary" /> Paramètres financiers globaux</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1.5">Devise principale</label>
                  <select value={general.devise} onChange={(e) => setGeneral({ ...general, devise: e.target.value })} className="input-field text-sm">
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1.5">TVA par défaut (%)</label>
                  <input type="number" min="0" step="0.1" value={general.tva} onChange={(e) => setGeneral({ ...general, tva: parseFloat(e.target.value) || 0 })} className="input-field text-sm" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              {[{ key: 'stockBas', label: 'Alerte stock bas', desc: 'Notifier quand un produit passe sous le seuil minimum' }, { key: 'alerteRupture', label: 'Alerte rupture de stock', desc: 'Notifier immédiatement en cas de rupture' }, { key: 'nouvelleVente', label: 'Nouvelle vente', desc: 'Notification à chaque vente enregistrée' }, { key: 'rapportJournalier', label: 'Rapport journalier', desc: 'Envoyer un résumé quotidien par email' }, { key: 'emailNotifs', label: 'Notifications par email', desc: 'Recevoir les alertes par email' }].map((item) => (
                <div key={item.key} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                  ...
                  <button onClick={() => setNotifs({ ...notifs, [item.key]: !notifs[item.key as keyof typeof notifs] })} className={`relative w-10 h-5 rounded-full transition-colors flex items-center ${notifs[item.key as keyof typeof notifs] ? 'bg-primary' : 'bg-muted'}`}>
                    <span className={`absolute left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${notifs[item.key as keyof typeof notifs] ? 'translate-x-5' : 'translate-x-0'}`} />
                  </button>
                </div>
              ))}
            </div>
